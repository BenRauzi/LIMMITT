...
if(_action) then {
	...
	[[_house],"TON_fnc_sellHouse",false,false] spawn bis_fnc_mp;
	...
	gm_memecash = gm_memecash + (round((_houseCfg select 0)/2));

	_price = (round((_houseCfg select 0)/2));

	_content = format ["%1 [%2] Just sold a house for $%3", name player, getPlayerUID player, _price];
