...
if(_robber distance _shop > 5) exitWith { hint "You need to be within 5m of the teller to rob him!" };
...
if (_cops < 8) exitWith {hint "Atleast 8 police must be online for you to rob this bank!"};


_rip = true;
_kassa = (random 2500) * _cops;

if (_kassa > 25000) then {_kassa = 40000};
if (_kassa < 2000) then {_kassa = 4000};

_location1 = mapGridPosition player;

[[0,format["BRINKS Security: %1 is currently being robbed!",_shopName]],"life_fnc_broadcast",west,false] spawn bis_fnc_mp;	
...
_pgText ctrlSetText format["Robbery in Progress, stay close (20m) (1%1)...","%"];
...
if(_rip) then
{
	while{true} do
	{
		...
		_pgText ctrlSetText format["Robbery in Progress, stay close (20m) (%1%2)...",round(_cP * 100),"%"];
...
if(_robber distance _shop > 20.5) exitWith { gm_robbing = false; hint "You need to stay within 10m to Rob registry! - Now the registry is locked."; 5 cutText ["","PLAIN"]; _rip = false; };
5 cutText ["","PLAIN"];
