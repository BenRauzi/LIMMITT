...
if((typeOf _container) in ["Box_IND_Grenades_F","B_supplyCrate_F"]) exitWith {
	...
	[[_house],"TON_fnc_updateHouseContainers",false,false] spawn bis_fnc_mp;
};

if (!gm_containerInUse) then {
_container setVariable ["GM_InUse", false, true];
if (vehicle player != player) then { vehicle player setVariable ["GM_InUse", false, true]; };
} else {
