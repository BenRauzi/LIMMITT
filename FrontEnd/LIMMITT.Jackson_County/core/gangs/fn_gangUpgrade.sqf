...
if(_action) then {
	if(gm_memecash < _upgradePrice) exitWith {
		hint parseText format[
			...
			[gm_memecash] call life_fnc_numberText,
			[_upgradePrice - gm_memecash] call life_fnc_numberText
		];
	};
	__SUB__(gm_memecash,_upgradePrice);
