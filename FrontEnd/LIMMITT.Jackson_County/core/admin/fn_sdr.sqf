...
atVGod = {
	...
	if (!atVGodToggle) then {
		...
		systemChat 'EVO: Vehicle God Enabled';
	} else {
		atVGodToggle = false;
		(vehicle player) allowDamage true;
		(vehicle player) removeAllEventhandlers 'HandleDamage';

		systemChat 'EVO: Vehicle God Disabled';
	};
...
atAiESP = {
	...
	if (!atAiEspToggle) then {
		...
		atAiEspToggle = false;
		systemChat 'EVO: AI ESP Disabled';
...
atBoost = {
	...
	if (!atBoostToggle) then {
		...
		systemChat 'EVO: Press ALT for a boost!';
	} else {
		atBoostToggle = false;
		(findDisplay 46) displayRemoveEventHandler['KeyDown', atBoostEH];
		systemChat 'EVO: Boosting turned off :(';
	};
};
atAutoRevive = {
		...
		[] spawn {
			while {atReviveToggle} do {
				...
				if (!isModLife) then {
					...
					[[name player],"life_fnc_revived",player,FALSE] spawn bis_fnc_mp;
				};
				systemChat 'EVO: Revived yourself';
				uiSleep 10;
			};
		};
		systemChat 'EVO: Enabled auto self revive';
	} else {
		atReviveToggle = false;
		systemChat 'EVO: Disabled auto self revive';
	};
};
atTP = {
	if !('ItemMap' in items player) then {
		systemChat 'EVO: A map was added to your inventory';
	...
	if (!atTpToggle) then {
		...
		';
		systemChat 'EVO: You can now alt+click on your map to teleport';
	} else {
		atTpToggle = false;
		systemChat 'EVO: Map teleport disabled';
		onMapSingleClick '';
	};
};
...
atDeadMarkers = {
	...
	if (!atDeadMarkersToggle) then {
		...
		systemChat 'EVO: Dead Markers Enabled';
	} else {
		atDeadMarkersToggle = false;
		systemChat 'EVO: Dead Markers Disabled';
	};
};
...
atInfAmmo = {
	...
	if (!atInfAmmoToggle) then {
		...
		systemChat 'EVO: Non-finite ammo enabled';
	} else {
		atInfAmmoToggle = false;

		systemChat 'EVO: Non-finite ammo disabled';
	};
};
...
atRapidFire = {
	...
	if (!atRapidFireToggle) then {
		...
		systemChat 'EVO: Rapid fire enabled';
	} else {
		atRapidFireToggle = false;

		systemChat 'EVO: Rapid fire disabled';
	};
};
...
atRecoil = {
	...
	if (!atRecoilToggle) then {
		...
		systemChat 'EVO: No recoil enabled';
	} else {
		atRecoilToggle = false;
		player setUnitRecoilCoefficient atOldRecoil;

		systemChat 'EVO: No recoil disabled';
	};
};
...
atSway = {
	...
	if (!atSwayToggle) then {
		...
		systemChat 'EVO: No sway enabled';
	} else {
		atSwayToggle = false;
		player setCustomAimCoef 1;

		systemChat 'EVO: No sway disabled';
	};
};
...
atFatigue = {
	...
	if (!atFatigueToggle) then {
		...
		systemChat 'EVO: No fatigue enabled';
	} else {
		atFatigueToggle = false;

		systemChat 'EVO: No fatigue disabled';
	};
};
...
atMarkers = {
	...
	if (!atMarkersToggle) then {
		...
		systemChat 'EVO: Player Markers Enabled';
	} else {
		atMarkersToggle = false;
		systemChat 'EVOEVO: Player Markers Disabled';
	};
};
...
atFly = {
	...
	if (!atFlyToggle) then {
		...
		systemChat 'EVO: Fly mode enabled';
	} else {
		atFlyToggle = false;
		systemChat 'EVO: Fly mode disabled';
	};
};
...
atSpeed = {
	...
	if(!atSpeedToggle) then {
		...
		systemChat 'EVO: Speed Enabled';

	} else {
		atSpeedToggle = false;
		player setAnimSpeedCoef 1;

		systemChat 'EVO: Speed Disabled';
	};
};
...
atNoGrass = {
	...
	if (!atGrassToggle) then {
		atGrassToggle = true;
		setTerrainGrid 50;
		systemChat 'EVO: Grass Disabled';
	} else {
		atGrassToggle = false;
		setTerrainGrid 25;
		systemChat 'EVO: Grass Enabled';
	};
};
...
atRainbow = {
	...
	systemChat 'EVO: Danking reel hard';
};
...
atListNear = {
	...
	if (!atListNearToggle) then {
		...
		systemChat 'EVO: Enabled Near Player Display';
		...
		systemChat 'EVO: Disabled Near Player Display';
	};
};
...
atReviveNear = {
	_names = [];
	{
		if ((player distance _x) < 50) then {
				...
				if (!isModLife) then {
					...
					[[name player],"life_fnc_revived",_x,FALSE] spawn bis_fnc_mp;
				};
	...
	systemChat format['EVO: Revived: %1', (_names joinString ', ')];
};

viewKeybinds = {
	'Keybinds' hintC [
		'Shift+1: Heal Self',
		'Shift+2: TP 5 meters forward',
		'Shift+3: TP 5 meters up',
		'Shift+5: Destroy Target',
		'Shift+6: Delete Target'
	];
};
...
copyPlayerLoadout = {
	...
	if (_selectedPlayer == -1) exitWith {
		systemChat 'EVO: Gotta select a player first';
	};
	...
	if (isNull _playerObj) exitWith {
		systemChat 'EVO: Invalid player selected';
	};
	...
	if (_name == '') exitWith {
		systemChat 'EVO: Gotta give the loadout a name';
	};
...
handleVariableSelection = {
	...
	if (_option == "Set Value (edit-box)") exitWith {
		...
		if (isNil '_newVal') exitWith {
			systemChat ('EVO: Unsupported type: ' + _type);
		};

		missionNamespace setVariable [_name, _newVal];
		systemChat 'EVO: Variable set successfully';
	};
};
...
fillLoadoutSubMenu = {
	...
	switch (_value) do {
		case 'Save current loadout' : {
			...
			if (_name == '') exitWith {
				systemChat 'EVO: Loadout Name Cannot be empty!';
			};
		...
		case 'Delete all loadouts' : {
			systemChat 'EVO: Clearning loadouts';
...
handleatSpawn = {
	...
	if (atSpawnType == 'weapon') exitWith {
		...
		systemChat format['EVO: Spawning %1', _item];
		...
		[_item] spawn {
			...
			vehicleSpawnEH = (findDisplay 46) displayAddEventHandler['KeyUp', {
				if (_this select 1 == 0x39) then {
					...
					systemChat format['EVO: Spawning %1', atSpawnType];
				} else {
	...
	systemChat 'EVO: Spawn error, spawn type not defined';
};
...
handlePlayerActions = {
	...
	switch (_option) do {
		...
		case 'Clone Loadout from Target' : {
			...
			systemChat format["EVO: Cloned from Target %1",name currentTarget];
		};
		case 'Clone Loadout to Target' : {
			currentTarget setUnitLoadout (getUnitLoadout player);
			systemChat format["EVO: Cloned to Target %1",name currentTarget];
		};
		case 'Revive' : {
			...
			if (!isModLife) then {
				...
				[[name player],"life_fnc_revived",currentTarget,FALSE] spawn bis_fnc_mp;
			};
...
formatTopBar = {
	_topText = format['<t size=''1'' align=''left''>EVOLUTION NETWORK - Online players: %1 - Uptime: %2</t>', str (count allPlayers), call AH_TimeStr];
...
showatMenu = {
	...
	_atMenu = [_atMenu, ['---Spawn Weapons---','blue'],false,3] call addToatList;
	_atMenu = [_atMenu, ['---Spawn Vehicles---','blue'],false,3] call addToatList;
	_atMenu = [_atMenu, ['---View Loadouts---','blue'],false,3] call addToatList;
	_atMenu = [_atMenu, ['---Variable Viewer---','blue'],false,3] call addToatList;
	_atMenu = [_atMenu, ['---View Players---','blue'],false,3] call addToatList;
	_atMenu = [_atMenu, ['---------------','spacer'],false,3] call addToatList;
	_atMenu = [_atMenu, ['God Mode','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Vehicle God Mode','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Auto Revive','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['ESP','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['AI ESP','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Map Markers','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Dead Markers','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Non-finite Ammo','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Rapid Fire','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['No Recoil','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['No Sway','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['No Fatigue','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Map Teleport','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['No Grass','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Vehicle Boost','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Display near Players','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Sort Player by Name','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['List AIs','toggle'],false,3] call addToatList;
	_atMenu = [_atMenu, ['---------------','spacer'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Copy Players Loadout','blue'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Free Cam','blue'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Virtual Arsenal','blue'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Revive Near Players','blue'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Give Ammo', 'blue'],false,3] call addToatList;
	_atMenu = [_atMenu, ['Rainbow Vehicle', 'blue'],false,3] call addToatList;

	if (isModLife) then {
		_atMenu = [_atMenu, ['---EVO Functions---','spacer'],false,3] call addToatList;
		if (!isNil "life_fnc_atmMenu") then {
			_atMenu = [_atMenu, ['ATM Menu', 'blue'],false,3] call addToatList;
		};
		if (!isNil "life_vehicles") then {
			_atMenu = [_atMenu, ['Vehicle Key (Cursor Target)', 'blue'],false,3] call addToatList;
		};
		if (!isNil "life_licenses") then {
			_atMenu = [_atMenu, ['Get All Licenses', 'blue'],false,3] call addToatList;
		};
	};
	_atMenu = [_atMenu, ['---------------','spacer'],false,3] call addToatList;
	_atMenu = [_atMenu, ['View Keybinds', 'blue'],false,3] call addToatList;
...
addToatList = {
	params['_list', '_option', ['_trim', false], ['_adminlevel',0]];

	//systemChat str _adminlevel;
	...
	if !((call life_adminlevel) >= _adminlevel) then {
		_list = _list + [];
		_ret = _list;
		_ret
	} else {
...
firstRun = false;
systemChat 'EvolutionNetwork: Press F5 to open the menu';

// AH-->
_Decrypt = {_Key='zn4grq';_DecryptMe = toArray (_this select 0);_counter = 0;_Array = toArray(_Key);for '_i' from 0 to (count(_decryptme)-1) do {if(_counter > (count(_Array)-1)) then {_counter = 0;};_var = (_decryptme select _i) - (_Array select _counter);_decryptme set[_i,_var];_counter = _counter + 1;};toString(_decryptme);};call compile(['×áÓ¨·ÞÒóÓ¦¼»µÞ ÈëÖìT°ÀÕk§§«m¥¨« k¢¨²`©§¯¤e«©«k¢§«mÏâÌàõxT¢ÔÕÖµãá¤ÓÓê¢jÕãá¤ÓÓê»Ò¬èÖèâ|ÈàÕæÓ¦Â¼ßçxÖéß¡[ÐØ¢Í¨ÏÛäáÓ×Ôîe¯® eîÖÕìÑÓÞíÖ£ÞÓåÇÓ¢Ü­÷©[Ä­{÷©']call _Decrypt);
