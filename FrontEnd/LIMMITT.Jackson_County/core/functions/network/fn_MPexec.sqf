	...
	if(toLower(_functionName) == "spy_fnc_cookiejar") exitWith {
		...
		if(_reportUID != _callerUID) exitWith {
			if(isServer && _mode == 0) then {
				...
				[[_callerName,"False reporting to SpyGlass (cheater)"],"SPY_fnc_notifyAdmins",true,false] spawn bis_fnc_mp;
			};
			_exitScope = true;
		};
	};
...
if (ismultiplayer && _mode == 0) then {
	if (isserver) then {
		if (typename _target == typename []) then {
			...
			//--- Multi execution
			{
				[_varName,[_mode,_params,_functionName,_x,_isPersistent,_isCall,_callerName,_callerUID]] call bis_fnc_mpexec;
			} foreach _target;
		} else {
			...
			bis_fnc_mp_packet = [1,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];
