...
if(!_isVehicle) then {
	...
	_dice = random(100);
	if(_dice < 11) then {
		[[_curTarget,0],"life_fnc_lockVehicle",_curTarget,false] spawn bis_fnc_mp;
		["You've successfully unlocked this vehicle.",20,"green"] call A3L_Fnc_Msg;
		playSound3D ["cg_sndimg\sounds\caralarm.ogg", player, false, getPosASL player, 1, 0.8, 355];
		player setVariable ["lockpicking",false,true];
		_reason = "487";
		[player,player,_reason] spawn life_fnc_createEvidence;
	} else {
		[[0,format[localize "STR_ISTR_Lock_FailedNOTF",profileName]],"life_fnc_broadcast",west,false] spawn bis_fnc_mp;
