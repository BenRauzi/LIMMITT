...
if(_value > 0) then {
	[[0,format[localize "STR_House_Raid_Successful",[_value] call life_fnc_numberText]],"life_fnc_broadcast",true,false] spawn bis_fnc_mp;
	gm_memecash = gm_memecash + _value;
	_house setVariable["Trunk",[_houseInvData,_houseInvVal],true];
	[[_house],"TON_fnc_updateHouseTrunk",false,false] spawn bis_fnc_mp;
} else {
