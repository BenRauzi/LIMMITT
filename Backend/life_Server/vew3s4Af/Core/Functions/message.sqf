...
_to = "The EMS";
[[_compiled,name player,5],"clientsmssys",independent,false] spawn bis_fnc_mp;
[] call life_fnc_cellphone;
} else { if (_to == 1) then {
_datecomp = format ["911 - (%6) %1-%2-%3 | %4:%5 %7",(date select 2),(date select 1),(date select 0),_hours,_mins,name player,_ampm];
_compiled = [name player,_msg,date,1,_datecomp];
_to = "The Police";
[[_compiled,name player,1],"clientsmssys",true,false] spawn bis_fnc_mp;
[] call life_fnc_cellphone;
} else { if (_to == 2) then {
_datecomp = format ["((%1 send the admins a message))",name player];
_compiled = [name player,_msg,date,2,_datecomp];
_to = "The Admins";
[[_compiled,name player,2],"clientsmssys",true,false] spawn bis_fnc_mp;
[] call life_fnc_cellphone;
} else { if (_to == 3 && ((call life_adminlevel) > 0)) then {
_datecomp = format ["((GLOBAL admin message from; %1))",name player];
_compiled = [name player,_msg,date,2,_datecomp];
_to = "EVERYONE";
[[_compiled,name player,4],"clientsmssys",true,false] spawn bis_fnc_mp;
[] call life_fnc_cellphone;
} else {
...
[[_compiled,name player,0],"clientsmssys",_to,false] spawn bis_fnc_mp;
[] call life_fnc_cellphone;
};};};
};};